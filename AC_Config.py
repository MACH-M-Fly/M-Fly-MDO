...
class AC():
	...
	def update_prop():
		...
		for num_wing in self.wing['Num']:
			wing_sref = 0.0
			key_start = 'W' + str(i+1)
			height = self.wing[key_start]['wingspan']/(self.wing['W'+str(i+1)]['num_sections']-1)
			for i in range(self.wing['W'+str(i+1)]['num_sections']-1):
	...
	def add_wing(name, airfoil, root_chord, wingspan, num_sections , offset):
	...
	def add_h_tail(name, airfoil, root_chord, wingspan, num_sections ):
		...
		new_wing['taper'] = size_array_1
		new_wing['dihedral'] = size_array_0
		wing_key = 'h_tail_' + str(self.wing['Num'] + 1)

		self.h_tail['wing_key'] = new_wing
		self.h_tail['Num'] = self.h_tail['Num'] + 1

	def add_v_tail(name, airfoil, root_chord, wingspan, num_sections ):
		''' Assumes symmetric h_tail '''
		# proper contains the following info in the following order:
		# Num_sections, Nchordwise, Cspace, Nspanwise, Sspace, COMPONENT, YDUPLICATE, ANGLE, SCALEX, SCALEY, SCALEZ, TRANSX, TRANSY, TRANSZ]
		# All airfoil, sweep, and taper variables need to be arrays size of the number of sections
		...
		new_wing['taper'] = size_array_1
		new_wing['dihedral'] = size_array_0
		wing_key = 'v_tail_' + str(self.wing['Num'] + 1)

		self.v_tail[wing_key] = new_wing
		self.v_tail['Num'] = self.v_tail['Num'] + 1

	def add_boom(density, length):

		self.boom['Boom_'+str(self.boom['Num']+!)] = {'boom_length' : length, 'density': density}
	...
	def create_AVL_geometry():
		with open(str(self.name)+'.avl', 'w') as geo:
			...
			for i in range(self.h_tail['Num']):
				geo.write('SURFACE\r\n')
				geo.write(str(self.h_tail['h_tail'+str(i+1)]['name']))
				geo.write('\r\n')
				geo.write('#Nchordwise\tCspace\tNspanwise\tSspace\r\n')
				geo.write(str(self.h_tail['h_tail'+str(i+1)]['proper'][0])+' '+str(self.h_tail['h_tail'+str(i+1)]['proper'][1])+' '+str(self.h_tail['h_tail'+str(i+1)]['proper'][2])+' '+str(self.h_tail['h_tail'+str(i+1)]['proper'][3])+'\r\n')
				geo.write('COMPONENT\r\n')
				geo.write(str(self.h_tail['h_tail'+str(i+1)]['proper'][4])+'\r\n')
				geo.write('YDUPLICATE\r\n')
				geo.write(str(self.h_tail['h_tail'+str(i+1)]['proper'][5])+'\r\n')
				geo.write('ANGLE\r\n')
				geo.write(str(self.h_tail['h_tail'+str(i+1)]['proper'][6])+'\r\n')
				geo.write('SCALE\r\n')
				geo.write(str(self.h_tail['h_tail'+str(i+1)]['proper'][7])+'\t'+str(self.h_tail['h_tail'+str(i+1)]['proper'][8])+'\t'+str(self.h_tail['h_tail'+str(i+1)]['proper'][9])+'\r\n')
				geo.write('TRANSLATE\r\n')
				geo.write(str(self.h_tail['h_tail'+str(i+1)]['proper'][10])+'\t'+str(self.h_tail['h_tail'+str(i+1)]['proper'][11])+'\t'+str(self.h_tail['h_tail'+str(i+1)]['proper'][12])+'\r\n')

				wingspan = self.h_tail['h_tail'+str(i+1)]['wingspan']
				num_sections = self.h_tail['h_tail'+str(i+1)]['num_sections']
				for j in range(num_sections):
					geo.write('SECTION\r\n')
					geo.write('#Xle\tYle\tZle\tChord\tAinc\tNspanwise\tSspace\r\n')
					geo.write(str(self.h_tail['h_tail'+str(i+1)]['X_offset'][j])+' '+str(wingspan* j / (num_sections - 1))+' '+str(math.sin(self.h_tail['h_tail'+str(i+1)]['dihedral'][j]) * wingspan / (num_sections - 1) +' '+str(self.h_tail['h_tail'+str(i+1)]['taper'][j] * self.h_tail['h_tail'+str(i+1)]['root_chord'])+' '+str(self.h_tail['h_tail'+str(i+1)]['angle'][j])+'\r\n')
					geo.write('AFILE\r\n')
					geo.write(str(self.h_tail['h_tail'+str(i+1)]['airfoil'][j])+'\r\n\r\n'))
			...
			for i in range(self.wing['Num']):
				geo.write('SURFACE\r\n')
				geo.write(str(self.wing['v_tail_'+str(i+1)]['name']))
				geo.write('\r\n')
				geo.write('#Nchordwise\tCspace\tNspanwise\tSspace\r\n')
